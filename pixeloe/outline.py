...
from .utils import sigmoid, apply_chunk_numpy


def expansion_weight(img, k=8, stride=2, avg_scale=10, dist_scale=3):
    img_y = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)[:, :, 0] / 255

    avg_y = apply_chunk_numpy(
        img_y, k * 2, stride, lambda x: np.median(x, axis=1, keepdims=True)
    )
    max_y = apply_chunk_numpy(
        img_y, k, stride, lambda x: np.max(x, axis=1, keepdims=True)
    )
    min_y = apply_chunk_numpy(
        img_y, k, stride, lambda x: np.min(x, axis=1, keepdims=True)
    )
